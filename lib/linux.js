...
module.exports = async () => {
	try {
		const {stdout: activeWindowIdStdout} = await execFile(xpropBin, xpropActiveArgs, { timeout: 1000 });
		...
		const [{stdout}, {stdout: boundsStdout}] = await Promise.all([
			execFile(xpropBin, xpropDetailsArgs.concat([activeWindowId]), { timeout: 1000 }),
			execFile(xwininfoBin, xpropDetailsArgs.concat([activeWindowId]), { timeout: 1000 })
		]);
